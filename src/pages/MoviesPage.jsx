    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                setMovieList(allContent.filter((c) => c.type !== "series"));
            } catch (err) {
    ...
    const derived = useMemo(() => {
        ...
        switch (sort) {
            ...
            case "year":
                ...
                break;
            // "recent" fallback leaves original order (assumed already recent)
            default:
                break;
        }
        return xs;
    }, [movieList, sort, genre]);
            ...
            <header className="mt-0 backdrop-blur supports-[backdrop-filter]:bg-slate-900/60 border-b border-white/10">
                <Container>
                    <div className="py-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                        <div>
                            <h1 className="text-xl md:text-2xl font-semibold tracking-tight">All Movies</h1>
                            <p className="text-sm text-slate-400">Browse and filter the complete collection of movies.</p>
                        </div>
